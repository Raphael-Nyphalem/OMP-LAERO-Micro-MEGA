...
char GPS_tab_pt1[2+sizeof(UTC)+sizeof(Longitude)];
char GPS_tab_pt2[2+sizeof(Latitude)+sizeof(AltitudeGPS)];
...
void loop() {
  ...
  if (radio.available()) {
    ...
    Serial.print("ARduino Cpt : ");
    Serial.println(Cpt_tab);
...
//                      + strlen(Hum) + strlen(Pres) + strlen(Alt) + sizeof(UTC) + sizeof(Longitude) + sizeof(Latitude) + sizeof(AltitudeGPS) + 20);    //+ size_uint16_t(data_outADC2_CH9)
//
